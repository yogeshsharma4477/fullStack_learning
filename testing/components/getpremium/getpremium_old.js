import styles from '../../styles/getpremium_old.module.scss';
...
export default function Getpremium({ sid, mobile, shorturl, docid }) {
  ...
  function RedirectGetPremiumPlan() {
    ...
    let baseurl = location.host && location.host.match(/\.jdsoftware\.jd/) ? 'http://project01.anilkumar.jdsoftware.jd' : '';
      ...
      <div className={`${styles.congratulation} container__inner flex__col flex__item__center`}>
        <div className={`${styles.congratulation__box}`}>
          <header className={`${styles.header}`}>
            <button className={`iconwrap backicon ripple`} />
          </header>
          {/* <span onClick={() => { window.location.href = window.location = 'https://www.justdial.com/' }} className={`${styles.congratulation__skip}`}>SKIP</span> */}
          <span onClick={(e) => RedirectJdBusinessFunctionCall(e, sourceCode, StoreCommonInfo?.docid)} className={`${styles.congratulation__skip}`}>SKIP</span>
          <div className={`${styles.congratulation__wrap}`}>
            <Image className={`${styles.congratulation__icon}`} src="//akam.cdn.jdmagicbox.com/images/icontent/listingbusiness/congratulation_white.svg" width={100} height={100} />
            <div className={`${styles.congratulation__hd}`}>Congratulations!</div>
          </div>
          <div className={`${styles.congratulation__sub}`}>You have successfully uploaded your additional information</div>
        </div>
        <div className={`${styles.congratulation__prembox}`}>
          ...
          <div className={`${styles.congratulation__premiumtxt}`}>Get Premium</div>
          <div className={`${styles.congratulation__premiumsubtxt}`}>Upgrade to Premium Listing Plan to rank higher on search results and get more customers to contact you.</div>
        </div>
